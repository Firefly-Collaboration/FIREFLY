...
===================================================================================
 <> Module: firefly_MANGA.py

 <> Author:
	- Justus Neumann ~ <jusneuma.astro__at__gmail.com>

 <> Contributors:
	- Daniel Goddard ~ <daniel.goddard__at__port.ac.uk>
    - Daniel Thomas ~ <daniel.thomas__at__port.ac.uk>
	- Samuel Helps ~ <samuel.helps.sh__at__gmail.com>

 <> Purpose:
	- Reads in a MaNGA datacube and analyses each spectrum from Voronoi binned
      spectra.
    - The bin number is optional and should only be given if only a single bin is
      meant to be fitted. (Created on Tue May 12 11:00:35 2020)
...
sys.path.append(os.path.join(repo_root, "Fitting_Engine"))
os.environ["FF_DIR"] = repo_root
os.environ["STELLARPOPMODELS_DIR"] = os.path.join(repo_root, "stellar_population_models")
