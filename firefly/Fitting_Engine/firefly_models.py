...
===================================================================================
 <> Module: firefly_models.py

 <> Author:
	- Johan Comparat ~ <johan.comparat__at__gmail.com>

 <> Contributors:
	- Sofia Meneses-Goytia ~ <s.menesesgoytia__at__gmail.com>
	- Violeta Gonzalez-Perez ~ <violegp__at__gmail.com>
	- Harry Hicks ~ <iamhrh__at__hotmail.co.uk>
	- Justus Neumann ~ <jusneuma.astro__at__gmail.com>
	- Daniel Thomas ~ <d.g.thomas__at__leeds.ac.uk>

 <> Purpose:
	- The class StellarPopulationModel is a wrapper dedicated to handling the fit
	  of stellar population models on observed spectra. 
...
from firefly_library import (
    airtovac, vactoair, convert_chis_to_probs, light_weights_to_mass,
    calculate_averages_pdf, normalise_spec, match_data_models
)
import matplotlib.pyplot as plt

# Define the models folder relative to this script
...
#				else:
#					model_path 		= join(MODELS_DIR,'SSP_M11_'+ model_used + '_downgraded', 'ssp_M11_' +model_used +'.' + imf_used)
#			else:
#				if model_used == 'MILES_UVextended' or model_used == 'MILES_revisedIRslope':
#					model_path 		= join(MODELS_DIR,'SSP_M11_MILES', 'ssp_M11_'+model_used+'.'+imf_used)
#				else:
