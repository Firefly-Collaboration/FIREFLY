...
def get_SFD_dust(long, lat, dustmap = 'ebv', interpolate=True):
	...
	* 'mask'
	...
	if dml in ('ebv', 'eb-v', 'e(b-v)'):
		dustmapfn = os.path.join(data_dir, 'SFD_dust_4096_%s.fits')
	elif dml == 'i100':
		dustmapfn = os.path.join(data_dir, 'SFD_i100_4096_%s.fits')
	elif dml == 'x':
		dustmapfn = os.path.join(data_dir, 'SFD_xmap_%s.fits')
	elif dml == 't':
		dustmapfn = os.path.join(data_dir, 'SFD_temp_%s.fits')
	elif dml == 'mask':
		dustmapfn = os.path.join(data_dir, 'SFD_mask_4096_%s.fits')
	else:
		dustmapfn = dustmap 
